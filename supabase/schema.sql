...
-- Billing entitlements are written only by service-role Stripe Edge Functions.
-- The browser anon key must never be able to grant, alter, or remove paid access.
...
create policy "subscriptions_update" on public.user_subscriptions for update using (false) with check (false);
...
-- Invoice history is generated by Stripe webhooks with the service-role key.
