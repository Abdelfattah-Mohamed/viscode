...
async function stripeGet(path: string): Promise<Record<string, unknown>> {
  ...
  if (!res.ok || data?.error) {
    const message = data?.error?.message || `Stripe API request failed (${res.status})`;
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const session = await stripeGet(`/checkout/sessions/${encodeURIComponent(eventSession.id)}`) as {
          id?: string;
          status?: string;
          mode?: string;
          customer?: unknown;
      ...
      case "customer.subscription.deleted": {
        const eventSub = event.data?.object as {
          id?: string;
        };
      ...
      case "invoice.paid": {
        ...
        const { data: subRow } = await supabase
          .from("user_subscriptions")
          .select("user_id")
          .eq("stripe_customer_id", stripeId(invoice.customer))
          .maybeSingle();
        if (subRow?.user_id) {
