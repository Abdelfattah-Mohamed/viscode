...
export const PROBLEMS = {
  ...
  "search-rotated-sorted": { title: "Search in Rotated Sorted Array", difficulty: "Medium", category: "Binary Search", timeComplexity: "O(log n)", spaceComplexity: "O(1)", visualizer: "array", description: "Given a rotated sorted array and <code>target</code>, return its index or -1.", example: { input: "nums = [4,5,6,7,0,1,2], target = 0", output: "4", note: "Target 0 is at index 4." }, defaultInput: { nums: [4, 5, 6, 7, 0, 1, 2], target: 0 }, inputFields: ["nums", "target"], explanation: [{ emoji: "🔍", title: "Binary Search", body: "Find which half is sorted; if target is in that range, search there; else the other half." }], languages: { cpp: { code: ["int search(vector<int>& nums, int target) {","    int l = 0, r = nums.size() - 1;","    while (l <= r) {","        int m = l + (r-l)/2;","        if (nums[m] == target) return m;","        if (nums[l] <= nums[m]) {","            if (target >= nums[l] && target < nums[m]) r = m - 1;","            else l = m + 1;","        } else {","            if (target > nums[m] && target <= nums[r]) l = m + 1;","            else r = m - 1;","        }","    }","    return -1;","}"], lineMap: { init: 1, done: 14 } }, java: { code: ["public int search(int[] nums, int target) {","    int l = 0, r = nums.length - 1;","    while (l <= r) { int m = (l+r)/2;","        if (nums[m] == target) return m;","        if (nums[l] <= nums[m]) {","            if (target >= nums[l] && target < nums[m]) r = m - 1;","            else l = m + 1;","        } else {","            if (target > nums[m] && target <= nums[r]) l = m + 1;","            else r = m - 1;","        }","    }","    return -1;","}"], lineMap: { init: 1, done: 13 } }, javascript: { code: ["function search(nums, target) {","  let l = 0, r = nums.length - 1;","  while (l <= r) { const m = (l+r)>>1;","    if (nums[m] === target) return m;","    if (nums[l] <= nums[m]) {","      if (target >= nums[l] && target < nums[m]) r = m - 1;","      else l = m + 1;","    } else {","      if (target > nums[m] && target <= nums[r]) l = m + 1;","      else r = m - 1;","    }","  }","  return -1;","}"], lineMap: { init: 1, done: 14 } }, python: { code: ["def search(nums, target):","    l, r = 0, len(nums)-1","    while l <= r:","        m = (l+r)//2","        if nums[m] == target: return m","        if nums[l] <= nums[m]:","            if nums[l] <= target < nums[m]: r = m-1","            else: l = m+1","        else:","            if nums[m] < target <= nums[r]: l = m+1","            else: r = m-1","    return -1"], lineMap: { init: 1, done: 12 } } } },
  "sum-two-integers": { title: "Sum of Two Integers", difficulty: "Medium", category: "Bit Manipulation", timeComplexity: "O(1)", spaceComplexity: "O(1)", visualizer: "array", description: "Calculate <code>a + b</code> without using + or - operator (use bit manipulation).", example: { input: "a = 1, b = 2", output: "3", note: "Use XOR for sum, AND for carry." }, defaultInput: { nums: [1, 2] }, inputFields: ["nums"], explanation: [{ emoji: "⚙️", title: "XOR & AND", body: "a ^ b gives sum without carry; (a & b) << 1 gives carry. Repeat until carry is 0." }], languages: { cpp: { code: ["int getSum(int a, int b) {","    while (b != 0) {","        int carry = (a & b) << 1;","        a = a ^ b;","        b = carry;","    }","    return a;","}"], lineMap: { init: 1, done: 7 } }, java: { code: ["public int getSum(int a, int b) {","    while (b != 0) {","        int carry = (a & b) << 1;","        a = a ^ b;","        b = carry;","    }","    return a;","}"], lineMap: { init: 1, done: 7 } }, javascript: { code: ["function getSum(a, b) {","  while (b !== 0) {","    const carry = (a & b) << 1;","    a = a ^ b;","    b = carry;","  }","  return a;","}"], lineMap: { init: 1, done: 7 } }, python: { code: ["def get_sum(a, b):","    mask = 0xFFFFFFFF","    while b & mask:","        carry = (a & b) << 1","        a, b = a ^ b, carry","    return a & mask if b > 0 else a"], lineMap: { init: 1, done: 5 } } } },
  "number-of-1-bits": { title: "Number of 1 Bits", difficulty: "Easy", category: "Bit Manipulation", timeComplexity: "O(1)", spaceComplexity: "O(1)", visualizer: "array", description: "Return the number of set bits (Hamming weight) in a 32-bit integer.", example: { input: "n = 11 (binary 1011)", output: "3", note: "Three 1s." }, defaultInput: { nums: [11] }, inputFields: ["nums"], explanation: [{ emoji: "⚙️", title: "n & (n-1)", body: "n & (n-1) clears the lowest set bit. Count until n is 0." }], languages: { cpp: { code: ["int hammingWeight(uint32_t n) {","    int count = 0;","    while (n) { n &= n - 1; count++; }","    return count;","}"], lineMap: { init: 1, done: 4 } }, java: { code: ["public int hammingWeight(int n) {","    int count = 0;","    while (n != 0) { n &= n - 1; count++; }","    return count;","}"], lineMap: { init: 1, done: 4 } }, javascript: { code: ["function hammingWeight(n) {","  let count = 0;","  while (n) { n &= n - 1; count++; }","  return count;","}"], lineMap: { init: 1, done: 4 } }, python: { code: ["def hamming_weight(n):","    count = 0","    while n: n &= n - 1; count += 1","    return count"], lineMap: { init: 1, done: 4 } } } },
  "counting-bits": { title: "Counting Bits", difficulty: "Easy", category: "Bit Manipulation", timeComplexity: "O(n)", spaceComplexity: "O(n)", visualizer: "array", description: "Return an array of length <code>n+1</code> where ans[i] is the number of 1s in binary representation of i.", example: { input: "n = 2", output: "[0,1,1]", note: "0→0, 1→1, 2→10." }, defaultInput: { n: 2 }, inputFields: ["n"], explanation: [{ emoji: "⚙️", title: "DP", body: "ans[i] = ans[i >> 1] + (i & 1)." }], languages: { cpp: { code: ["vector<int> countBits(int n) {","    vector<int> ans(n+1);","    for (int i = 1; i <= n; i++)","        ans[i] = ans[i >> 1] + (i & 1);","    return ans;","}"], lineMap: { init: 1, done: 5 } }, java: { code: ["public int[] countBits(int n) {","    int[] ans = new int[n+1];","    for (int i = 1; i <= n; i++)","        ans[i] = ans[i >> 1] + (i & 1);","    return ans;","}"], lineMap: { init: 1, done: 5 } }, javascript: { code: ["function countBits(n) {","  const ans = Array(n+1).fill(0);","  for (let i = 1; i <= n; i++) ans[i] = ans[i>>1] + (i & 1);","  return ans;","}"], lineMap: { init: 1, done: 4 } }, python: { code: ["def count_bits(n):","    ans = [0] * (n+1)","    for i in range(1, n+1): ans[i] = ans[i>>1] + (i & 1)","    return ans"], lineMap: { init: 1, done: 4 } } } },
  "reverse-bits": { title: "Reverse Bits", difficulty: "Easy", category: "Bit Manipulation", timeComplexity: "O(1)", spaceComplexity: "O(1)", visualizer: "array", description: "Reverse the bits of a 32-bit unsigned integer.", example: { input: "n = 00000010100101000001111010011100", output: "964176192", note: "Reversed bits." }, defaultInput: { nums: [43261596] }, inputFields: ["nums"], explanation: [{ emoji: "⚙️", title: "Bit by bit", body: "Extract LSB with n & 1, add to result, shift n right and result left." }], languages: { cpp: { code: ["uint32_t reverseBits(uint32_t n) {","    uint32_t res = 0;","    for (int i = 0; i < 32; i++) {","        res = (res << 1) | (n & 1);","        n >>= 1;","    }","    return res;","}"], lineMap: { init: 1, done: 7 } }, java: { code: ["public int reverseBits(int n) {","    int res = 0;","    for (int i = 0; i < 32; i++) {","        res = (res << 1) | (n & 1);","        n >>>= 1;","    }","    return res;","}"], lineMap: { init: 1, done: 7 } }, javascript: { code: ["function reverseBits(n) {","  let res = 0;","  for (let i = 0; i < 32; i++) {","    res = (res << 1) | (n & 1);","    n >>>= 1;","  }","  return res >>> 0;","}"], lineMap: { init: 1, done: 6 } }, python: { code: ["def reverse_bits(n):","    res = 0","    for _ in range(32):","        res = (res << 1) | (n & 1)","        n >>= 1","    return res"], lineMap: { init: 1, done: 6 } } } },
  ...
  "top-k-frequent": { title: "Top K Frequent Elements", difficulty: "Medium", category: "Arrays", timeComplexity: "O(n)", spaceComplexity: "O(n)", visualizer: "array", description: "Given an integer array and k, return the k most frequent elements.", example: { input: "nums = [1,1,1,2,2,3], k = 2", output: "[1,2]", note: "Bucket sort or quickselect." }, defaultInput: { nums: [1, 1, 1, 2, 2, 3], k: 2 }, inputFields: ["nums", "k"], explanation: [{ emoji: "🗂️", title: "Bucket sort", body: "Count freq; bucket[i] = numbers with freq i; scan from high bucket." }], languages: { cpp: { code: ["vector<int> topKFrequent(vector<int>& nums, int k) {","    unordered_map<int,int> count;","    for (int x : nums) count[x]++;","    vector<vector<int>> bucket(nums.size()+1);","    for (auto& [num, c] : count) bucket[c].push_back(num);","    vector<int> res;","    for (int i = bucket.size()-1; i >= 0 && res.size() < k; i--)","        for (int x : bucket[i]) { res.push_back(x); if (res.size() == k) break; }","    return res;","}"], lineMap: { init: 1, done: 10 } }, java: { code: ["public int[] topKFrequent(int[] nums, int k) {","    Map<Integer, Integer> count = new HashMap<>();","    for (int x : nums) count.merge(x, 1, Integer::sum);","    List<List<Integer>> bucket = new ArrayList<>();","    for (int i = 0; i <= nums.length; i++) bucket.add(new ArrayList<>());","    for (Map.Entry<Integer,Integer> e : count.entrySet()) bucket.get(e.getValue()).add(e.getKey());","    int[] res = new int[k]; int j = 0;","    for (int i = nums.length; i >= 0 && j < k; i--)","        for (int x : bucket.get(i)) { res[j++] = x; if (j == k) return res; }","    return res;","}"], lineMap: { init: 1, done: 12 } }, javascript: { code: ["function topKFrequent(nums, k) {","  const count = {};","  for (const x of nums) count[x] = (count[x]||0) + 1;","  const bucket = Array(nums.length+1).fill(null).map(() => []);","  for (const [num, c] of Object.entries(count)) bucket[c].push(+num);","  const res = [];","  for (let i = bucket.length-1; i >= 0 && res.length < k; i--)","    for (const x of bucket[i]) { res.push(x); if (res.length === k) return res; }","  return res;","}"], lineMap: { init: 1, done: 10 } }, python: { code: ["def top_k_frequent(nums, k):","    from collections import Counter","    count = Counter(nums)","    bucket = [[] for _ in range(len(nums)+1)]","    for num, c in count.items(): bucket[c].append(num)","    res = []","    for i in range(len(nums), -1, -1):","        for x in bucket[i]: res.append(x);","        if len(res) == k: return res"], lineMap: { init: 1, done: 8 } } } },
  ...
  "min-stack": { title: "Min Stack", difficulty: "Medium", category: "Stack", timeComplexity: "O(1) per op", spaceComplexity: "O(n)", visualizer: "array", description: "Design a stack that supports push, pop, top, and getMin in O(1).", example: { input: "push -2, push 0, push -3, getMin, pop, top, getMin", output: "-3, 0, -2", note: "Aux stack or store min with each value." }, defaultInput: { nums: [1, 2, 3] }, inputFields: ["nums"], explanation: [{ emoji: "📚", title: "Two stacks or paired", body: "Keep a parallel stack of minimums, or store (val, minSoFar) in each node." }], languages: { cpp: { code: ["class MinStack {","    stack<int> st, minSt;","public:","    void push(int val) { st.push(val); minSt.push(minSt.empty() ? val : min(minSt.top(), val)); }","    void pop() { st.pop(); minSt.pop(); }","    int top() { return st.top(); }","    int getMin() { return minSt.top(); }","};"], lineMap: { init: 1, done: 6 } }, java: { code: ["class MinStack {","    Deque<Integer> st = new ArrayDeque<>();","    Deque<Integer> minSt = new ArrayDeque<>();","    public void push(int val) { st.push(val); minSt.push(minSt.isEmpty() ? val : Math.min(minSt.peek(), val)); }","    public void pop() { st.pop(); minSt.pop(); }","    public int top() { return st.peek(); }","    public int getMin() { return minSt.peek(); }","}"], lineMap: { init: 1, done: 7 } }, javascript: { code: ["class MinStack {","  constructor() { this.st = []; this.minSt = []; }","  push(val) { this.st.push(val); this.minSt.push(this.minSt.length ? Math.min(this.minSt[this.minSt.length-1], val) : val); }","  pop() { this.st.pop(); this.minSt.pop(); }","  top() { return this.st[this.st.length-1]; }","  getMin() { return this.minSt[this.minSt.length-1]; }","}"], lineMap: { init: 1, done: 6 } }, python: { code: ["class MinStack:","    def __init__(self): self.st = []; self.min_st = []","    def push(self, val): self.st.append(val); self.min_st.append(min(self.min_st[-1], val) if self.min_st else val)","    def pop(self): self.st.pop(); self.min_st.pop()","    def top(self): return self.st[-1]","    def getMin(self): return self.min_st[-1]"], lineMap: { init: 1, done: 6 } } } },
  "eval-rpn": { title: "Evaluate Reverse Polish Notation", difficulty: "Medium", category: "Stack", timeComplexity: "O(n)", spaceComplexity: "O(n)", visualizer: "array", description: "Evaluate expression given as RPN (postfix) tokens: numbers and +, -, *, /.", example: { input: "tokens = [\"2\",\"1\",\"+\",\"3\",\"*\"]", output: "9", note: "Stack: push numbers; on op pop two, compute, push result." }, defaultInput: { s: "2,1,+,3,*" }, inputFields: ["s"], explanation: [{ emoji: "📚", title: "Stack", body: "Push numbers; on operator pop two, apply, push result." }], languages: { cpp: { code: ["int evalRPN(vector<string>& tokens) {","    stack<int> st;","    for (auto& t : tokens) {","        if (t == \"+\" || t == \"-\" || t == \"*\" || t == \"/\") {","            int b = st.top(); st.pop(); int a = st.top(); st.pop();","            if (t == \"+\") st.push(a+b); else if (t == \"-\") st.push(a-b); else if (t == \"*\") st.push(a*b); else st.push(a/b);","        } else st.push(stoi(t));","    }","    return st.top();","}"], lineMap: { init: 1, done: 9 } }, java: { code: ["public int evalRPN(String[] tokens) {","    Deque<Integer> st = new ArrayDeque<>();","    for (String t : tokens) {","        if (\"+*-/\".contains(t)) {","            int b = st.pop(), a = st.pop();","            if (t.equals(\"+\")) st.push(a+b); else if (t.equals(\"-\")) st.push(a-b); else if (t.equals(\"*\")) st.push(a*b); else st.push(a/b);","        } else st.push(Integer.parseInt(t));","    }","    return st.pop();","}"], lineMap: { init: 1, done: 9 } }, javascript: { code: ["function evalRPN(tokens) {","  const st = [];","  for (const t of tokens) {","    if (['+','-','*','/'].includes(t)) {","      const b = st.pop(), a = st.pop();","      if (t === '+') st.push(a+b); else if (t === '-') st.push(a-b); else if (t === '*') st.push(a*b); else st.push((a/b)|0);","    } else st.push(Number(t));","  }","  return st.pop();","}"], lineMap: { init: 1, done: 9 } }, python: { code: ["def eval_rpn(tokens):","    st = []","    for t in tokens:","        if t in '+-*/':","            b, a = st.pop(), st.pop()","            st.append(int(eval(f'a{t}b')))","        else: st.append(int(t))","    return st[-1]"], lineMap: { init: 1, done: 7 } } } },
  "generate-parentheses": { title: "Generate Parentheses", difficulty: "Medium", category: "Backtracking", timeComplexity: "O(4^n / sqrt(n))", spaceComplexity: "O(n)", visualizer: "array", description: "Given n pairs of parentheses, generate all valid combinations.", example: { input: "n = 3", output: "[\"((()))\",\"(()())\",\"(())()\",\"()(())\",\"()()()\"]", note: "Backtrack: add ( when open < n; add ) when close < open." }, defaultInput: { n: 3 }, inputFields: ["n"], explanation: [{ emoji: "🔙", title: "Backtrack", body: "Only add ) if close < open; only add ( if open < n." }], languages: { cpp: { code: ["void backtrack(int n, int open, int close, string path, vector<string>& res) {","    if (path.size() == 2*n) { res.push_back(path); return; }","    if (open < n) backtrack(n, open+1, close, path + '(', res);","    if (close < open) backtrack(n, open, close+1, path + ')', res);","}","vector<string> generateParenthesis(int n) {","    vector<string> res;","    backtrack(n, 0, 0, \"\", res);","    return res;","}"], lineMap: { init: 1, done: 7 } }, java: { code: ["void backtrack(int n, int open, int close, StringBuilder path, List<String> res) {","    if (path.length() == 2*n) { res.add(path.toString()); return; }","    if (open < n) { path.append('('); backtrack(n, open+1, close, path, res); path.setLength(path.length()-1); }","    if (close < open) { path.append(')'); backtrack(n, open, close+1, path, res); path.setLength(path.length()-1); }","}","public List<String> generateParenthesis(int n) {","    List<String> res = new ArrayList<>();","    backtrack(n, 0, 0, new StringBuilder(), res);","    return res;","}"], lineMap: { init: 1, done: 8 } }, javascript: { code: ["function backtrack(n, open, close, path, res) {","  if (path.length === 2*n) { res.push(path); return; }","  if (open < n) backtrack(n, open+1, close, path+'(', res);","  if (close < open) backtrack(n, open, close+1, path+')', res);","}","function generateParenthesis(n) {","  const res = [];","  backtrack(n, 0, 0, '', res);","  return res;","}"], lineMap: { init: 1, done: 7 } }, python: { code: ["def backtrack(n, open, close, path, res):","    if len(path) == 2*n: res.append(path); return","    if open < n: backtrack(n, open+1, close, path+'(', res)","    if close < open: backtrack(n, open, close+1, path+')', res)","def generate_parenthesis(n):","    res = []; backtrack(n, 0, 0, '', res)","    return res"], lineMap: { init: 1, done: 6 } } } },
