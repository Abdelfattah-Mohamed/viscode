...
export default function IntervalsViz({ stepState = {}, problemId, t }) {
  const { intervals = [], merged = [], current = -1, done, lastEnd, count, removedIndices = [] } = stepState;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {intervals.length > 0 && (
        <div>
          <div style={{ fontFamily: "'Caveat',cursive", fontSize: "0.95em", fontWeight: 600, color: t.inkMuted, marginBottom: 10 }}>intervals (sorted by end)</div>
          {renderBars(intervals, false, isNonOverlapping ? { markRemoved: removedIndices } : {})}
        </div>
      )}
