export default function PalindromicSubstringsViz({ s = "", stepState = {}, t = {} }) {
  ...
  const isL = (idx) => idx === left && left >= 0 && left < n;
  const isR = (idx) => idx === right - 1 && right > 0 && right <= n && right > left + 1;

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      <div style={{ fontFamily: "'Caveat',cursive", fontSize: "0.95em", fontWeight: 600, color: t.inkMuted, marginBottom: 4 }}>
        Expand around center: for each center i try odd (i,i) and even (i,i+1), expand while s[l]==s[r], count each palindrome.
      </div>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div style={{ padding: "8px 14px", border: `2px solid ${t.border}`, borderRadius: 8, background: t.surfaceAlt }}>
          <span style={{ fontFamily: "'Caveat',cursive", color: t.inkMuted, marginRight: 6 }}>count</span>
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, color: t.green }}>{count}</span>
        </div>
        {type && (
      ...
      <div>
        ...
        {palindrome && !done && (
          <div style={{ marginTop: 8, fontFamily: "'Caveat',cursive", fontSize: "0.9em", color: t.inkMuted }}>
            Current palindrome: <span style={{ color: t.green, fontWeight: 700 }}>&quot;{palindrome}&quot;</span>
          </div>
        )}
      </div>
      ...
      {done && (
        <div style={{ padding: "12px 18px", borderRadius: 10, border: `2px solid ${t.green}`, background: t.green + "22", fontFamily: "'JetBrains Mono',monospace", fontSize: "1.05em", fontWeight: 800, color: t.green }}>
          Answer: {count} palindromic substrings
        </div>
      )}
    </div>
  );
