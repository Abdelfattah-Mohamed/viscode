export default function BinarySearchViz({ nums, stepState = {}, t }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      <div>
        <div style={{ fontFamily: "'Caveat',cursive", fontSize: "0.9rem", color: t.inkMuted, marginBottom: 10 }}>nums (sorted)</div>
            ...
            return (
              <div key={idx} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, position: "relative" }}>
                <div style={{ position: "absolute", bottom: "100%", marginBottom: 8, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                  {idx === left  && found === -1 && <span style={{ fontFamily: "'Caveat',cursive", fontSize: "0.85rem", fontWeight: 700, color: t.blue }}>L</span>}
                  {idx === right && found === -1 && <span style={{ fontFamily: "'Caveat',cursive", fontSize: "0.85rem", fontWeight: 700, color: t.purple }}>R</span>}
                  {isMid   && <span style={{ fontFamily: "'Caveat',cursive", fontSize: "0.85rem", fontWeight: 700, color: t.yellow }}>mid</span>}
                  {isFound && <span style={{ fontFamily: "'Caveat',cursive", fontSize: "1rem",    fontWeight: 700, color: t.green  }}>found</span>}
                </div>
                <div style={{ width: 54, height: 54, border: `2px solid ${isFound ? t.green : isMid ? t.yellow : t.border}`, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Caveat',cursive", fontSize: "1.3rem", fontWeight: 700, background: isFound ? t.green + "44" : isMid ? t.yellow + "44" : t.surface, color: isElim ? t.inkMuted : t.ink, opacity: isElim ? 0.25 : 1, transform: isFound || isMid ? "translateY(-8px) scale(1.1)" : "scale(1)", transition: "all 0.3s" }}>{val}</div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        {[{ label: "L", val: left  >= 0 ? left  : "—", color: t.blue   },
          ...
          <div key={x.label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", border: `2px solid ${t.border}`, borderRadius: 10, background: t.surfaceAlt }}>
            ...
            <span style={{ fontFamily: "'Caveat',cursive", fontSize: "1.5rem", fontWeight: 700, color: x.color }}>{x.val}</span>
          </div>
        ))}
        {found >= 0 && <div style={{ display: "flex", alignItems: "center", padding: "8px 16px", border: `2px solid ${t.green}`, borderRadius: 10, background: t.green + "18" }}><span style={{ fontFamily: "'Caveat',cursive", fontSize: "1.05rem", fontWeight: 700, color: t.green }}>✅ Found at index {found}</span></div>}
      </div>
    </div>
  );
}
